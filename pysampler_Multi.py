...
##kick = AudioSegment.from_file("carbon.wav")
...
class track:
    def __init__(self, master):
        ...
        ##filedialog code: (needs fixing)
        
        self.sample_filename = filedialog.askopenfilename(initialdir=".", title="Load Sample", filetypes=(("wav files", "*.wav"),))

        self.sample_label = Label(root, text=self.sample_filename).grid(row=5, column=0)


        self.sample = AudioSegment.from_file(self.sample_filename)
...
def eventLoop(L):
    ...
    print(status)
    i = 0
    for item in L:##you need to create a thread? for each sample or 
        print(item)
        print(type(item))
